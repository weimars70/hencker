  ...
  <q-page padding>
    <div class="role-page">
            ...
            <q-card class="role-card">
              <q-card-section>
                <div class="row items-center no-wrap">
                  <div class="col">
                    <div class="text-subtitle1 text-weight-bold">{{ role.nombre }}</div>
                    <div class="text-caption text-grey">{{ role.codigo }}</div>
                  </div>
      ...
      <div v-if="!loading && filteredRoles.length === 0" class="empty-state">
        <q-icon name="admin_panel_settings" size="4rem" color="grey-4" />
        <div class="text-h6 text-grey-6 q-mt-md">No hay roles disponibles</div>
        <div class="text-grey-6 q-mb-md">Agregue un nuevo rol para comenzar</div>
        <q-btn
          color="primary"
          icon="add"
          label="Nuevo Rol"
...
const filteredRoles = computed(() => {
  ...
  return roles.value.filter(role => 
    role.nombre.toString().toLowerCase().includes(search) ||
    role.codigo.toString().toLowerCase().includes(search)
  );
});
...
const saveRole = async (formData: Partial<Role>) => {
  saving.value = true;
  try {
    if (editingRole.value) {
      await roleRepository.updateRole({
        codigo: editingRole.value.codigo,
...
const confirmDelete = (role: Role) => {
  $q.dialog({
    ...
    try {
      await roleRepository.deleteRole(role.codigo);
      await loadRoles();
...
.role-grid-enter-active,
.role-grid-leave-active {
  transition: all 0.5s ease;
}

.role-grid-enter-from,
.role-grid-leave-to {
  opacity: 0;
  transform: scale(0.9);
}
